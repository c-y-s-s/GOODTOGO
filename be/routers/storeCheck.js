...
const storage = multer.diskStorage({
  ...
  destination: function (req, file, cb) {
    ...
    cb(
      null,
      path.join(__dirname, "..", "..", "fe", "src", "images", "store_img")
    );
    cb(
      null,
      path.join(__dirname, "..", "..", "be", "public", "uploads", "stores")
    );
    // 錯誤訊息 先給null
  },
  // 設定儲存的檔名
...
router.post(
  ...
  async (req, res, next) => {
    ...
    let filename1 = req.files.storeLogo[0].filename;
    console.log("加上路徑的 filename: ", filename1);

    let filename2 = req.files.storeLicence[0].filename;
    console.log("加上路徑的 filename: ", filename2);

    let hashpassword = await argon2.hash(req.body.password);

    // -------- 儲存到資料庫 --------
    let [updateProfileResult] = await connection
      .execute(
        `INSERT INTO stores (bossname, name, email, account, password, tel_no, address, open_time, close_time,close_day,logo,certification_img,stores_category_id, valid) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);`,
        [
          req.body.name,
          req.body.storeName,
          req.body.email,
      ...
      .then(
        connection.execute(`UPDATE stores SET close_day='[6,7]' WHERE id > 76`)
      );
