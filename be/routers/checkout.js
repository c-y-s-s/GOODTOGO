...
router.get("/:storeId", async (req, res, next) => {
  let [data, fields] = await connection.execute(
    `SELECT a.*,
      ...
      b.price ,
      b.img,
      b.amount AS product_now_amount
...
//!
router.post("/deleteproductsamount", async (req, res, next) => {
  req.body.forEach(async (item) => {
    let [result] = await connection.execute(
      `UPDATE products SET amount=? WHERE id = ?`,
      [item.productsNowAmount - item.amount , item.productsId]
    )
  });
  res.json({ msg: "Checkout Products UPDATE ok" });
});
